...
if ( ! class_exists( 'OPS_Resize' ) ) {
    ...
    class OPS_Resize {
        ...
        /**
         * Should an OPS_Exception be thrown on error?
        ...
        /**
         * For your custom default usage you may want to initialize an OPS_Resize object by yourself and then have own defaults
         */
        ...
        public function process( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false ) {
            try {
                // Validate inputs.
                if (!$url)
                    throw new OPS_Exception('$url parameter is required');
                if (!$width)
                    throw new OPS_Exception('$width parameter is required');
                if (!$height)
                    throw new OPS_Exception('$height parameter is required');
                ...
                if ( false === strpos( $url, $upload_url ) )
                    throw new OPS_Exception('Image must be local: ' . $url);
                ...
                if ( ! file_exists( $img_path ) or ! getimagesize( $img_path ) )
                    throw new OPS_Exception('Image file does not exist (or is not an image): ' . $img_path);
                ...
                if ( ! $dims && ( ( ( null === $height && $orig_w == $width ) xor ( null === $width && $orig_h == $height ) ) xor ( $height == $orig_h && $width == $orig_w ) ) ) {
                    ...
                    if ( ! $dims || ( true == $crop && false == $upscale && ( $dst_w < $width || $dst_h < $height ) ) ) {
                        ...
                        throw new OPS_Exception('Unable to resize image because image_resize_dimensions() failed');
                    }
                    ...
                    // Else, we resize the image and return the new resized image url.
                        ...
                        if ( is_wp_error( $editor ) || is_wp_error( $editor->resize( $width, $height, $crop ) ) ) {
                            throw new OPS_Exception('Unable to get WP_Image_Editor: ' . 
                        ...
                        if ( ! is_wp_error( $resized_file ) ) {
                            ...
                            throw new OPS_Exception('Unable to save resized image file: ' . $editor->get_error_message());
                        }
...
if ( ! function_exists( 'ops_resize' ) ) {

    /**
     * This is just a tiny wrapper function for the class above so that there is no
     * need to change any code in your own WP themes. Usage is still the same :)
     */
    function ops_resize( $url, $width = null, $height = null, $crop = null, $single = true, $upscale = false ) {
        /* WPML Fix */
        ...
        $ops_resize = OPS_Resize::getInstance();
        return $ops_resize->process( $url, $width, $height, $crop, $single, $upscale );
    }
}

if ( ! function_exists( 'ops_image_attributes' ) ) {

    /**
     * Build our image attributes
     */
    function ops_image_attributes( $og_width = '', $og_height = '', $new_width = '', $new_height = '' ) {
        ...
        return apply_filters( 'ops_image_attributes', $img_atts );
    }
}